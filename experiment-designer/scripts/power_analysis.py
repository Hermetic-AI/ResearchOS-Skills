...
t-distribution's slightly heavier tails, so treat small-n results (n < ~20)
as mildly optimistic.
...
def main(argv=None):
    ...
    if args.solve in ("n", "power"):
        d = effect_from_args(args, args.test)
        if d <= 0:
    ...
    if args.solve == "n":
        ...
        result["power"] = round(power_from(d, args.n, args.alpha, args.sides), 4)
    else:
        if not args.n:
    ...
    notes = [
        "normal approximation; mildly optimistic for n < ~20",
