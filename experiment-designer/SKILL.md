...
description: Helps researchers design experiments BEFORE data collection — recommends the right design type (completely randomized / randomized block / full or fractional factorial / response surface / crossover / within-subject), reviews threats to validity, designs control groups, produces reproducible randomization and stratified block assignments, estimates statistical power / required sample size, and plans ML/AI experiments (baselines, ablations, hyperparameter budgets, seed replication, benchmark leakage). Use when someone is planning a study or experiment, asks "how should I set up this experiment", "帮我设计实验方案", "实验该怎么分组/设对照", "怎么随机分组/区组/分层", "需要多少样本量/做功效分析", "因子实验/DOE 怎么排", "消融实验怎么做", "baseline 怎么选", "效度威胁/内部效度", mentions controls, randomization, blocking, factorial designs, sample size, power analysis, ablation studies, or benchmark leakage. NOT for analyzing data already collected — use data-analysis-assistant instead; NOT for reading/summarizing papers — use literature-reader instead; NOT for writing manuscript text — use paper-writing-assistant instead; NOT for building knowledge graphs — use knowledge-graph-builder instead.
---
...
2. **Factor-space exploration** — enumerate, fractionate, or response-surface the factor combinations to run.
...
5. **Power / sample-size estimation** — a priori n for t-tests (independent / one-sample / paired), two-proportion, and correlation tests.
...
3. **Generate the factor layout** (multi-factor experiments): run `scripts/doe_designs.py` (requires numpy — `pip install numpy`). Full/fractional factorial, Box-Behnken, central composite, or Latin square run sheets in real units with seeded run order, as CSV/JSON. **Ablation matrices:** run `scripts/ablation_planner.py` (zero dependencies) for leave-one-out / full / cumulative ablation run matrices.
...
5. **Estimate sample size / power**: run `scripts/power_analysis.py` (zero dependencies, normal approximation) for two-sample / one-sample / paired t-tests, two proportions, or correlations. Always report a sensitivity range over plausible effect sizes, not a single n; warn against post-hoc ("observed") power.
...
- `scripts/doe_designs.py` — full / fractional factorial, Box-Behnken, central composite, Latin square generator (numpy); seeded; CSV/JSON.
...
- `scripts/power_analysis.py` — sample size / power / MDE for t-tests (independent, one-sample, paired), two proportions, correlations (stdlib only, normal approximation).
