...
Note: double quotes inside emitted values are replaced with single quotes —
the minimal frontmatter parser does not support escaped quotes.
"""
...
def yaml_value(value):
    """Emit a double-quoted YAML scalar safe for the minimal parser."""
...
def already_declared(fm_lines, relation, target):
    ...
    for chunk in block.split("- relation: "):
        if not chunk.startswith(relation):
            continue
