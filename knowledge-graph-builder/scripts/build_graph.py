#!/usr/bin/env python3
"""Build a typed concept graph from a directory of Markdown literature notes.
...
---------
    ...
    notes_dir        Directory to scan recursively for .md files.
...
NODE_TYPES = {"paper", "method", "dataset", "task", "metric", "topic", "note"}
...
class GraphBuilder:
    ...
    def add_edge(self, frm, to, relation, origin, evidence):
        key = (frm, to, relation, evidence.get("source"), evidence.get("line"))
        for edge in self.edges:
            if (edge["from"], edge["to"], edge["relation"],
                    edge["evidence"].get("source"),
                    edge["evidence"].get("line")) == key:
                return
    ...
    def to_dot(self):
        shapes = {"paper": "box", "method": "ellipse", "dataset": "cylinder",
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Build a typed concept graph from Markdown notes.")
    parser.add_argument("notes_dir", nargs="?",
                        help="directory of Markdown notes")
