...
description: 面向在读硕士/博士的论文写作辅助。三大功能：(1) 看图写正文分析段落——输入图/表图片，生成论文正文里"如图X所示……"的分析文字；(2) 引用规范检查——正文引用与参考文献表一一对应、每条格式合规、字段完整、风格一致（内置 GB/T 7714、IEEE、APA、ACM，也可按学校/期刊要求文档自带的引用格式检查，并提示校方要求与国标的冲突/过时/不全）；(3) 论文格式检查——用户粘贴自然语言格式要求（或指一份要求文件），解析成检查清单并确认后，比对论文实际排版（Word .docx 全查，LaTeX 只查源码可静态核实项）。支持 LaTeX 源码与 Word .docx。当用户提到"帮我写图表分析/看图写段落""检查论文引用/参考文献格式""按学校要求检查论文格式/排版""论文格式核对"等时触发。
---
...
- **(b) 用户给了「学校/期刊要求文档」，要按它查**（如华东理工 §9、某期刊投稿须知）：按功能三的读法读要求文件（`.doc`→`textutil`，`.docx`→`docx_text.py`，`.pdf`→Read/`pdftotext`），把其中的参考文献格式**解析成一份「校方风格规格」**（字段顺序、分隔标点、是否带类型码 `[J]`、作者缩写规则、卷期页写法、给了哪些文献类型的模板），发给用户确认。**这份校方规格优先于内置风格作为检查基准。**
...
1. **收要求**：用户粘贴自然语言要求（如"正文小四宋体，行距 1.5 倍，页边距上下 2.5cm 左右 3cm，一级标题黑体三号居中，图表题注五号"），或指一份要求文件——按后缀选读法：
   ...
   - **`.doc`（旧二进制 OLE2，脚本读不了）**：用 macOS 自带 `textutil -convert txt -stdout <要求.doc>` 抽文本（无 textutil 时退 `antiword`/`catdoc`）。
...
## 文件索引
- `scripts/docx_inspect.py` — 零依赖提取 .docx 真实排版（中英文字体分列/字号/行距/页边距/标题样式/题注含加粗）。
