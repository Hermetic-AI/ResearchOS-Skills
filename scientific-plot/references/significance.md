...
- **Three or more groups**: overall `f_oneway` (one-way ANOVA) first; if p < 0.05, pairwise two-sample t-tests for the bracket pairs — `--control NAME` compares every group against the control, otherwise all pairs (capped at 6 brackets to keep the figure readable; refine the pair list by editing the CSV or running twice with subsets).
...
1. For each compared pair (g1, g2) at x positions p1, p2: base height = max(top of the two error bars, i.e. mean + error).
