...
def check_mermaid(src):
    ...
    for i, line in enumerate(lines[1:], 2):
        # strip legal mermaid node-shape wrappers before looking for bad labels:
        # A[((x))] double-circle-in-bracket, A[(x)] cylinder, A[[x]] subroutine,
        # A{{x}} hexagon, A((x)) circle
        stripped = re.sub(r"\[\(\(.*?\)\)\]|\[\(.*?\)\]|\[\[.*?\]\]|\{\{.*?\}\}|\(\(.*?\)\)",
                          "", line)
...
def main():
    args = parse_args()
    if args.out:
        if args.src != "-" and os.path.abspath(args.out) == os.path.abspath(args.src):
            sys.exit("error: --out must not replace the source diagram")
        if os.path.exists(args.out) and not args.force:
            sys.exit(f"error: output exists: {args.out}; use --force to replace it")
