...
def require_scientific_stack():
    """Load optional numerical dependencies only for tests that need them."""
    global np, stats
    ...
    try:
        ...
        raise SystemExit(2)
    np, stats = numpy_module, stats_module
...
def fmt_p(p):
    return f"p = {p:.4g}" if p >= 0.0001 else "p < 0.0001"
...
def to_artifact(result, args):
    """Map native output to the versioned ResearchOS stat-results contract."""
    source = ({"kind": "file", "locator": os.path.abspath(args.file)}
              if args.file else {"kind": "user", "locator": "CLI p-value list"})
    if "statistic" in result:
        ...
        rows = [{
            ...
            "adjusted_p_value": float(row["p_adjusted"]),
    ...
    return {
        ...
        "provenance": {
            "created_by": "data-analysis-assistant/stat_test.py",
            "tool_version": "0.1.0",
            "command": " ".join(sys.argv),
            "seed": None,
            "sources": [source],
            "warnings": list(result.get("reminders", [])),
        },
...
def write_artifact(result, args):
    if not args.artifact_out:
        return
    ...
    with open(args.artifact_out, "w", encoding="utf-8", newline="\n") as handle:
        json.dump(to_artifact(result, args), handle, ensure_ascii=False, indent=2)
        handle.write("\n")
...
def to_markdown(r):
    ...
    if "statistic" in r:
        lines.append(f"- Statistic: {r['statistic']} | {fmt_p(r['p_value'])} | "
...
def main(argv=None):
    ...
    ap.add_argument("--artifact-out", help="write a schema-versioned stat-results JSON artifact")
    ap.add_argument("--result-id", default="primary", help="stable comparison id for artifact output")
    ap.add_argument("--force", action="store_true", help="replace an existing --artifact-out file")
    args = ap.parse_args(argv)

    if args.test == "adjust":
        r = run_adjust(args)
        write_artifact(r, args)
