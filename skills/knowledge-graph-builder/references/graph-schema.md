...
The graph is a property graph over Markdown notes and versioned `paper-note` JSON. **The fact source is the notes and their evidence anchors**; the JSON/DOT output of `build_graph.py` is a rebuildable projection, never the source of truth.
...
| `claim` | One atomic claim from validated `paper-note.claims[]` | `paper-note` claim ID + evidence anchors |
...
Node identity: `note:<vault-relative path>` for Markdown files, `paper:<citekey or normalized DOI>` for papers, a stable title/year/author hash when a paper-note lacks DOI, `claim:<paper-id>:<claim-id>` for claims, and `concept:<lowercased name>` for concept targets. Merge case variants, plural/singular, and frontmatter `aliases:` entries into one node — but only after user confirmation.
...
| `supports` | paper → claim | each imported `paper-note` evidence anchor | ✅ schema-derived; one edge per anchor |
