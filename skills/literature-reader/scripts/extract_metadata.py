...
PMID_RE = re.compile(
    r"(?:\bPMID\s*[:：]?\s*|pubmed\.ncbi\.nlm\.nih\.gov/)(\d{1,9})\b",
    re.IGNORECASE,
)
...
def extract_arxiv_version(text):
    m = ARXIV_RE.search(text)
...
def extract_title(text, doi, warnings=None):
    """Best-effort title.
    ...
    If the chosen segment still starts with multiple "Surname, I." name pairs
    (i.e. the author filter failed, e.g. on unusual initials), a warning is
    appended instead of silently returning the author string as the title.
    """
    ...
    for seg in segments:
        ...
        if len(seg.split()) < 2:
            continue
        if warnings is not None and re.match(
            r"^(?:[A-Z][A-Za-z'`-]+,\s*(?:[A-Z]\.-?\s*)+,?\s*(?:and\s+|&\s*)?){2,}", seg
        ):
            warnings.append(
                "title candidate starts with multiple author names, likely an "
                "author list misread as title: '%s'" % seg[:80]
            )
        return seg
    return None
...
def parse_bibtex(text):
    ...
    for idx, m in enumerate(BIB_ENTRY_RE.finditer(text)):
        ...
        entry = {
            ...
            "doi": fields.get("doi"),
            "arxiv_id": extract_arxiv(eprint or ""),
            "arxiv_version": extract_arxiv_version(eprint or ""),
            "pmid": fields.get("pmid") or fields.get("pubmed"),
            "year": year,
...
def main(argv=None):
    ...
    if mode == "auto":
        ...
        # Bare APA author-year entries ("Smith, J. (2020).") carry no label;
        # without this check a short list can fall through to single-blob mode.
        bare_apa_hits = sum(
            1 for ln in text.splitlines()
            if re.match(r"^[A-Z][A-Za-z'`-]+,.*\((?:19|20)\d{2}[a-z]?\)\.", ln.strip())
        )
        mode = "list" if (numbering != "none" or ay_hits >= 1 or bare_apa_hits >= 1
                          or year_hits >= 3) else "text"
    ...
    for idx, (_, label, body) in enumerate(raw_entries):
        ...
        entry = {
            ...
            "arxiv_version": extract_arxiv_version(body),
            "pmid": extract_pmid(full),
            "year": year,
            "title": extract_title(body, doi, warnings),
            "authors": authors,
            "citation_key": citation_key(authors, year),
        }
        if entry["title"] is None:
