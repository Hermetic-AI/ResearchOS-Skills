...
5. **Deliver**: the filled note, schema-valid `paper-note` JSON and `evidence-audit` JSON when files are available, plus a 3–5 sentence Chinese oral summary (口头速览). Disclose any unmatched quote or unverified OCR; never downgrade an audit failure to a prose footnote.
