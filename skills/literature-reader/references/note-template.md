...
### Evidence register
- Read `references/evidence-anchoring.md` before filling this section. Every core
  claim needs page or section, a short source quote, extraction method, verification
  state, and explicit direct/partial/context-only support strength.
- Run `scripts/audit_claim_evidence.py` against the `paper-note` JSON and, when
  available, its `pdf-extraction` JSON. A machine pass does not replace judgment.
